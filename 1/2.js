const result = async () => {
  ...
  entries.forEach(entryA => {
    [...entries].forEach(entryB => {
      ...
      if (entryC) {
        matches.push({ entryA, entryB, entryC })
      }
    })
  })
  const answer = matches.map(match => {
    match.sum = match.entryA + match.entryB + match.entryC
    match.product = match.entryA * match.entryB * match.entryC
    return match
  })
  console.log(answer);
}
